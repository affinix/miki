...
import {
    getUserEmojiUsageRank,
    getUserMostSent,
    getUserReactionMostSentTo,
...
const RSentCommand: ICommand = {
    commandName: "rsent",
    category: CommandCategory.REACTIONS,
    desc: "Ranks reaction emotes you've sent.",
    longDesc:
        "Detailed information about how many times you've sent each type of reaction.",
    admin: false,
    args: [{
        name: "user",
    ...
    exec: async (client, message, userMention) => {
        ...
        const generateSentPage = async (
            ...
            const leaderboard = await getUserMostSent(client, user.id);
            for (
                ...
                const usageRank = await getUserEmojiUsageRank(
                    client,
                    user.id,
                    entry.emoji,
                );
                const mostSentTo = (await getUserReactionMostSentTo(
                    client,
                    user.id,
                    entry.emoji,
                ))[0];
                ...
                const reactionInfo = new TextDisplayBuilder()
                    .setContent(
                        `⠀⠀↪⠀**\`#${usageRank?.rank}\` user on the server**⠀\n` +
            ...
            const footerText = new TextDisplayBuilder()
                .setContent(
                    `-# use \`!rsent [@user]\` to view info about other users!`,
                );
            displayPage
                .addSeparatorComponents(separator)
        ...
        const pagesNo = Math.ceil(
            (await getUserMostSent(client, user.id)).length / PAGE_LENGTH,
        );
        sendPaginatedMessage(
            client,
            message,
